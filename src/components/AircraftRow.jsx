...
export default function AircraftRow({ aircraft, onEdit }) {
  const { registration, manufacturer, model, status } = aircraft;
    ...
    <div className={`flex justify-between items-center ${bg} rounded-lg my-2 py-4 px-6 hover:shadow-xl hover:-translate-y-1 transition transform`}>
      <div className="flex items-center space-x-2">
        ...
        <div>
          ...
          <p className="text-sm text-gray-200">{manufacturer} - {model}</p>
        </div>
      </div>
      <div className="flex items-center space-x-4">
        <span className={`${pillBg} px-4 py-2 rounded-full font-semibold ${pillText} shadow-lg uppercase`}>{status.replace('_', ' ')}</span>
        <HiOutlinePencil className="text-white cursor-pointer" onClick={() => onEdit(aircraft)} />
      </div>
    </div>
  );
}
